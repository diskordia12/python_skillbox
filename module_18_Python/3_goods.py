...
for item_name, item_id in goods.items():
    result[item_name] = {'quantity': 0, 'price': 0}
    for item_info in store[item_id]:
        result[item_name]['quantity'] += item_info['quantity']
        result[item_name]['price'] += item_info['quantity'] * item_info['price']

for good, info in result.items():
    print(f'{good} - {info["quantity"]} pieces, cost {info["price"]:,d} rubs')
