...
Это может быть полезно, например, при анализе комментариев в социальных сетях или обработке
текстовых данных для исследований.
Задача
...
Советы
Вспомните методы строк, которые изучили.
Выберите из них те, которые помогут вам выполнить проверку буквы (является ли она заглавной или нет).
Чтобы вернуть несколько элементов из функции, перечислите их через запятую:
return a, b
"""


def count_uppercase_lowercase(text: str) -> int:
    upper_letters = 0
    lower_letters = 0
    for letter in text:
        if letter.isupper():
...
text = input('Line for analysis: ')
uppercase, lowercase = count_uppercase_lowercase(text)
print(f'Amount of uppercase letters: {uppercase}')
print(f'Amount of lowercase letters: {lowercase}')
